...
const RadioEmo = ({ view, value, setView, emotion, emoHover }: Props) => {
    return (
        <Radio
            value={`${emoHover}`}
            className={`relative flex justify-center items-center w-[100px] h-[100px] rounded-md ${
                value === emoHover ? 'shadow-xl' : ''
            }`}
        >
            <Image
                src={`/${emoHover}.png`}
            ...
            {view === emoHover && value !== emoHover ? (
                <div
                    className="absolute px-[12px] py-[5px] bg-[#b2a4d4] whitespace-nowrap bottom-[-20px] rounded-md left-[50%] translate-x-[-50%] shadow-xl text-white
